...
parser.add_argument("--no_attempt_load_state_dict", dest="attempt_load_state_dict", action="store_false")
parser.add_argument("--sample_size", type=int, default=50_000, help="Subsample the specified number of training data to train LightGBM model.", )
...
data, col_stats_dict = make_pkey_fkey_graph(dataset.get_db(), col_to_stype_dict=col_to_stype_dict,
...
if task.task_type == TaskType.BINARY_CLASSIFICATION:
    ...
    clamp_min, clamp_max = np.percentile(task.get_table("train").df[task.target_col].to_numpy(), [2, 98])
...
for split in ["train", "val", "test"]:
    ...
    loader_dict[split] = NeighborLoader(data, num_neighbors=[int(args.num_neighbors / 2 ** i) for i in range(args.num_layers)], time_attr="time", input_nodes=table_input.nodes,
                                        input_time=table_input.time, transform=table_input.transform, batch_size=args.batch_size, temporal_strategy=args.temporal_strategy,
                                        shuffle=split == "train", num_workers=args.num_workers, persistent_workers=args.num_workers > 0, )
...
@torch.no_grad()
def embed(model: Model, loader: NeighborLoader, no_label: bool = False, stop_at: int = None, ) -> Dict[str, float]:
    ...
    for idx, batch in enumerate(tqdm(loader)):
        batch = batch.to(device)
        embed = model_embed(batch, task.entity_table, )
...
# Model training
# =====================
model = Model(data=data, col_stats_dict=col_stats_dict, num_layers=args.num_layers, channels=args.channels, out_channels=out_channels, aggr=args.aggr, norm="batch_norm", ).to(
    device)
...
def tensor_to_tf(data, y=None):
    tf = torch_frame.TensorFrame(feat_dict={torch_frame.numerical: data}, col_names_dict={torch_frame.numerical: [f"feat_{i}" for i in range(data.shape[1])], }, )
    if y is not None:
        tf.y = y
...
relbench2torch_frame = {TaskType.MULTILABEL_CLASSIFICATION: TaskTypeTorchFrame.MULTILABEL_CLASSIFICATION, TaskType.BINARY_CLASSIFICATION: TaskTypeTorchFrame.BINARY_CLASSIFICATION,
