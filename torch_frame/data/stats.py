...
class StatType(Enum):
    ...
    def stats_for_stype(stype: torch_frame.stype) -> list[StatType]:
        stats_type = {torch_frame.numerical: [StatType.MEAN, StatType.STD, StatType.QUANTILES, ], torch_frame.categorical: [StatType.COUNT],
                      torch_frame.multicategorical: [StatType.MULTI_COUNT], torch_frame.sequence_numerical: [StatType.MEAN, StatType.STD, StatType.QUANTILES, ],
                      torch_frame.timestamp: [StatType.YEAR_RANGE, StatType.NEWEST_TIME, StatType.OLDEST_TIME, StatType.MEDIAN_TIME, ], torch_frame.embedding: [StatType.EMB_DIM, ]}
        return stats_type.get(stype, [])

    def compute(self, ser: Series, sep: str | None = None, ) -> Any:
        if self == StatType.MEAN:
            ...
            return np.quantile(flattened[finite_mask].astype(np.float32), q=[0, 0.25, 0.5, 0.75, 1], ).tolist()   # numpy boolean subtract, the `-` operator, is not supported
            ...
            ser = ser.apply(lambda row: MultiCategoricalTensorMapper.split_by_sep(row, sep))
            ...
            return TimestampTensorMapper.to_tensor(pd.Series(ser.iloc[-1])).squeeze(0)

        elif self == StatType.OLDEST_TIME:
            return TimestampTensorMapper.to_tensor(pd.Series(ser.iloc[0])).squeeze(0)

        elif self == StatType.MEDIAN_TIME:
            return TimestampTensorMapper.to_tensor(pd.Series(ser.iloc[len(ser) // 2])).squeeze(0)

        elif self == StatType.EMB_DIM:
            return len(ser[0])


_default_values = {StatType.MEAN: np.nan, StatType.STD: np.nan, StatType.QUANTILES: [np.nan, np.nan, np.nan, np.nan, np.nan], StatType.COUNT: ([], []),
                   StatType.MULTI_COUNT: ([], []), StatType.YEAR_RANGE: [-1, -1], StatType.NEWEST_TIME: torch.tensor([-1, -1, -1, -1, -1, -1, -1]),
                   StatType.OLDEST_TIME: torch.tensor([-1, -1, -1, -1, -1, -1, -1]), StatType.MEDIAN_TIME: torch.tensor([-1, -1, -1, -1, -1, -1, -1]), StatType.EMB_DIM: -1, }


def compute_col_stats(ser: Series, stype: torch_frame.stype, sep: str | None = None, time_format: str | None = None, ) -> dict[StatType, Any]:
    ...
    if ser.isnull().all():
        # NOTE: We may just error out here if eveything is NaN
        stats = {stat_type: _default_values[stat_type] for stat_type in StatType.stats_for_stype(stype)}
    else:
        if stype == torch_frame.timestamp:
            ser = pd.to_datetime(ser, format=time_format)
            ser = ser.sort_values()
        stats = {stat_type: stat_type.compute(ser.dropna(), sep) for stat_type in StatType.stats_for_stype(stype)}
